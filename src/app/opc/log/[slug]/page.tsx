...
export default async function OPCLogDetailPage(
  props: PageProps<"/opc/log/[slug]">
) {
  ...
  const renderContent = (content: string) => {
    return content.split("\n").map((line, i) => {
      ...
      if (/^\d+\.\s/.test(line)) return <li key={i} className="ml-4 text-text-secondary list-decimal">{line.replace(/^\d+\.\s/, "")}</li>;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-12">
      <Link href="/opc/log" className="text-sm text-text-muted hover:text-text transition-colors">
        ← 返回日志列表
      </Link>
      <div className="mt-6 flex items-center gap-3">
        <span className="text-sm text-text-muted font-mono">{entry.date}</span>
        <span className="inline-flex items-center rounded-full bg-[#818cf8]/10 px-2 py-0.5 text-xs text-[#818cf8]">{entry.mood}</span>
      </div>
      <div className="mt-4 flex gap-2">
        {entry.tags.map((tag) => (
          <span key={tag} className="rounded-md bg-bg-card px-2 py-1 text-xs text-text-muted">#{tag}</span>
        ))}
      </div>
