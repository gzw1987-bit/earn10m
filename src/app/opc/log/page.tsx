        ...
        {entries.map((entry) => (
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs ${moodColor[entry.mood] ?? "bg-bg text-text-muted"}`}>
                {entry.mood}
              </span>
            </div>
            ...
            <div className="mt-3 flex gap-2">
              {entry.tags.map((tag) => (
                <span key={tag} className="rounded-md bg-bg px-2 py-1 text-xs text-text-muted">#{tag}</span>
              ))}
            </div>
          </Link>
        ))}
